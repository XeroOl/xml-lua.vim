...
" Last Changed:	Dec 05, 2020
" Filenames:	*.xml
" Last Change:
" 20201205 - MODIFIED to embed lua into the xml strings.
...
"                      ^^^^^^^
syn region  xmlString contained start=+"\(%|(function\)\@!+ end=+"+ contains=xmlEntity,@Spell display
syn region  xmlString contained start=+'\(%|(function\)\@!+ end=+'+ contains=xmlEntity,@Spell display
...
if exists('g:xml_syntax_folding')
	...
	\ contained
	\ contains=xmlError,xmlTagName,xmlAttrib,xmlEqual,xmlLuaString,xmlString,@xmlStartTagHook
...
syn region  xmlProcessing matchgroup=xmlProcessingDelim start="<?" end="?>" contains=xmlAttrib,xmlEqual,xmlLuaString,xmlString
...
if exists('g:xml_syntax_folding')
	...
	\ fold
	\ contains=xmlDocTypeKeyword,xmlInlineDTD,xmlLuaString,xmlString
else
...
"highlight anything lua with lua
syn region xmlLuaString matchgroup=xmlString contained start=+"%+ end=+"+ contains=@Lua display
syn region xmlLuaString matchgroup=xmlString contained start=+'%+ end=+'+ contains=@Lua display
syn region xmlLuaString matchgroup=xmlString contained start=+"(function+ end=+end)()"+ contains=@Lua display
syn region xmlLuaString matchgroup=xmlString contained start=+'(function+ end=+end)()'+ contains=@Lua display
